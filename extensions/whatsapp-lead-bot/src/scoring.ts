...
export function isInSinaloa(location: string): boolean {
  const normalized = location.toLowerCase().trim();
  if (normalized.includes("sinaloa")) {
    return true;
  }
...
export function computeScore(input: ScoreInput): LeadScore | null {
  ...
  if (!isInSinaloa(input.location)) {
    return "OUT";
  }

  // Renters can't install panels
  if (input.ownership?.toLowerCase() === "rentada") {
    return "OUT";
  }

  // Score by bill amount
  if (input.bimonthly_bill < 500) {
    return "OUT";
  }
  if (input.bimonthly_bill < 1000) {
    return "COLD";
  }
  if (input.bimonthly_bill < 2000) {
    return "WARM";
  }
  return "HOT";
}

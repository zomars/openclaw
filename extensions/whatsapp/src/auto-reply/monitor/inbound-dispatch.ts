...
export async function buildWhatsAppInboundContext(params: {
  ...
  const inboundHistory =
    params.msg.chatType === "group"
      ...
      : params.dmHistory
        ? buildInboundHistoryFromEntries({
            entries: params.dmHistory.map((entry) => ({
              sender: entry.sender,
              body: entry.body,
              timestamp: entry.timestamp,
              messageId: entry.id,
            })),
            limit: params.dmHistory.length,
          })
        : undefined;
  ...
  return buildChannelInboundEventContext({
    ...
    extra: {
      ...
      // Fork patch: surface fromMe owner-sent messages so handoff detection downstream
      // can distinguish account-owner-sent replies from agent-sent ones.

...
export function validateCronForm(form: CronFormState): CronFieldErrors {
  ...
  } else if (!form.payloadLocked && !form.payloadText.trim()) {
    errors.payloadText =
  ...
  if (
    ...
    (!form.payloadLocked && form.payloadKind === "agentTurn")
  ) {
...
function jobToForm(job: CronJob, prev: CronFormState): CronFormState {
  ...
  const next: CronFormState = {
      ...
      payload?.kind === "systemEvent"
        ...
        : payload?.kind === "agentTurn"
          ...
          : payload?.kind === "command"
            ? payload.argv.join(" ")
            : "",
    scriptCommand: payload?.kind === "script" ? payload.command : "",
    scriptArgs: payload?.kind === "script" ? (payload.args ?? []).join(" ") : "",
    scriptCwd: payload?.kind === "script" ? (payload.cwd ?? "") : "",
    ...
    timeoutSeconds:
      (payload?.kind === "agentTurn" || payload?.kind === "script") &&
